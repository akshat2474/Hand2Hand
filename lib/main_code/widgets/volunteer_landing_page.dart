              ...
              TextButton(
                onPressed: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => const DonateWhat()));
                },
...
class _DonateWhatState extends State<DonateWhat> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            const Text("Donate What?", style: TextStyle(color: Colors.black)),
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (user != null) {
        try {
          ...
          // Update the user's donated meals count
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                    labelText: 'Quantity (number of meals)'),
                validator: (value) =>
      ...
      if (user != null) {
        try {
          ...
          await FirebaseFirestore.instance.runTransaction((transaction) async {
            ...
            Map<String, dynamic> userData =
                snapshot.data() as Map<String, dynamic>;
            List<dynamic> currentMiscellaneous =
                userData['miscellaneous'] ?? [];
            currentMiscellaneous
                .add('$selectedItem (${quantityController.text})');
            transaction
                .update(userRef, {'miscellaneous': currentMiscellaneous});
          });
...
class _DonateMoneyPageState extends State<DonateMoneyPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            const Text("Donate Money", style: TextStyle(color: Colors.black)),
        backgroundColor: Colors.white,
        elevation: 0,
        leading: IconButton(
